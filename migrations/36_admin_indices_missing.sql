...
-- admin/blog/suscriptores: ORDER BY creado_en DESC
create index if not exists suscriptores_blog_creado_idx
  on public.suscriptores_blog(creado_en desc);
...
-- eventos_log: el panel de logs filtra por cuenta_id + tipo + fecha
create index if not exists eventos_log_cuenta_tipo_idx
  on public.eventos_log(cuenta_id, tipo, creado_en desc)
