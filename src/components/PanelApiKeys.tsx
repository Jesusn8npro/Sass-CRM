 ...
 *
 * - Lista las keys existentes (nombre, prefijo, último uso, fecha).
 * - Botón "Crear nueva clave" → modal con nombre + permisos →
 *   crea, muestra plaintext UNA VEZ con botón "Copiar".
 * - Botón "Revocar" en cada fila con `useConfirm`.
...
export function PanelApiKeys({ idCuenta }: { idCuenta: string }) {
  ...
  const [claveRecienCreada, setClaveRecienCreada] = useState<{
    apiKey: ApiKey;
    plaintext: string;
  } | null>(null);
  ...
  async function cargar() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(
          "Las API Keys no están habilitadas todavía (falta la migración 10).",
        );
      }
    } finally {
      setCargando(false);
  ...
  async function revocar(id: string, nombre: string) {
    const ok = await confirmar({
      mensaje: `¿Revocar la clave "${nombre}"? Las integraciones que la estén usando dejarán de funcionar inmediatamente.`,
      textoConfirmar: "Revocar",
      variante: "peligro",
    });
    if (!ok) return;
    const res = await fetch(`/api/cuentas/${idCuenta}/api-keys/${id}`, {
      method: "DELETE",
    });
    if (res.ok) {
      toast.exito("Clave revocada");
      void cargar();
    } else {
      const d = (await res.json().catch(() => ({}))) as { error?: string };
      toast.error(d.error ?? "No se pudo revocar la clave");
    }
  }
  ...
  return (
    <div className="flex h-full flex-col">
        ...
        <div className="relative">
          <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
            <div className="flex items-start gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-lg text-white shadow-md">
                🔑
              </div>
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Generá claves para integrar tu CRM con Zapier, Make, n8n o
                  tu propio sistema.
                </p>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-500/30 dark:bg-amber-950/30">
            <div className="flex items-start gap-3">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300">
                ⚠
              </div>
              <div className="flex-1">
                <p className="text-sm font-semibold text-amber-900 dark:text-amber-100">
                  Tratá las claves como contraseñas
                </p>
                <p className="mt-0.5 text-xs text-amber-800 dark:text-amber-200">
                  Solo se muestra el valor completo <strong>UNA VEZ</strong> al
                  crearla. Después solo verás el prefijo. Si la perdés, tenés
                  que crear una nueva.
                </p>
              </div>
            </div>
        ...
        {cargando ? (
          ...
          <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-12 text-center dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <p className="mt-1 text-xs text-zinc-500">
              Tocá &quot;+ Crear nueva clave&quot; para generar la primera.
            </p>
          </div>
        ) : (
                  ...
                  >
                    <td className="px-4 py-3">
                      ...
                      <p className="text-[10px] text-zinc-500">
                        Creada{" "}
                        {new Date(k.creado_en).toLocaleDateString("es-AR")}
                      </p>
                    </td>
                    <td className="px-4 py-3">
                      <code className="rounded bg-zinc-100 px-2 py-0.5 font-mono text-[11px] text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                        {k.prefijo}…
                      </code>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {k.permisos.length === 0 ? (
                          ...
                          k.permisos.map((p) => (
                            <span
                              key={p}
                              className="inline-flex rounded-full bg-emerald-100 px-2 py-0.5 font-mono text-[10px] font-medium text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300"
                            >
                              {p}
                            </span>
                          ))
                    ...
                    <td className="px-4 py-3 text-xs text-zinc-600 dark:text-zinc-400">
                      {tiempoRelativo(k.ultima_uso_en)}
                    </td>
                    <td className="px-4 py-3">
                      <span
                      ...
                      >
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${activa(k) ? "bg-emerald-500" : "bg-zinc-400"}`}
                        />
                    ...
                    <td className="px-4 py-3">
                      {activa(k) ? (
                        <button
                          type="button"
                          onClick={() => revocar(k.id, k.nombre)}
                          className="rounded-md px-2 py-1 text-[11px] font-semibold text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40"
                        >
                          Revocar
                        </button>
                      ) : (
      ...
      {modalAbierto && (
        <ModalCrearKey
          ...
          onCreada={(d) => {
            ...
            setClaveRecienCreada({ apiKey: d.api_key, plaintext: d.clave_plaintext });
            void cargar();
          }}
        />
      )}
      ...
      {claveRecienCreada && (
        <ModalMostrarPlaintext
          plaintext={claveRecienCreada.plaintext}
          nombre={claveRecienCreada.apiKey.nombre}
          onCerrar={() => setClaveRecienCreada(null)}
        />
      )}
    </div>
  );
...
function ModalCrearKey({
  ...
  idCuenta: string;
  onCerrar: () => void;
  onCreada: (d: RespuestaCrear) => void;
}) {
  ...
  async function enviar() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(
          ("error" in data && data.error) || "No se pudo crear la clave",
        );
        return;
      }
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <p className="mt-1 text-xs text-zinc-500">
          Vas a poder copiar el valor completo solo una vez después de crearla.
        </p>
        ...
        <div className="mt-5 space-y-4">
          <div>
            ...
            <p className="mt-1 text-[10px] text-zinc-500">
              Etiqueta interna para reconocerla después.
            </p>
          </div>
            ...
            <div className="space-y-2">
              <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-zinc-200 px-3 py-2 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-900">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500">
                    Leer conversaciones e información de la cuenta.
                  </p>
                </div>
              </label>
              <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-zinc-200 px-3 py-2 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-900">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500">
                    Enviar mensajes salientes desde el bot.
                  </p>
                </div>
              </label>
            </div>
...
// ============================================================
// Modal: mostrar plaintext recién creada
// ============================================================
function ModalMostrarPlaintext({
  plaintext,
  nombre,
  onCerrar,
}: {
  ...
  async function copiar() {
    try {
      ...
      // Si falla el clipboard (https requerido), seleccionamos el texto.
    }
  }
      ...
      <div className="w-full max-w-lg rounded-2xl border border-emerald-300 bg-white p-6 shadow-2xl dark:border-emerald-500/40 dark:bg-zinc-950">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-zinc-500">
              <strong>{nombre}</strong> — copiala ahora, no se mostrará de
              nuevo.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5">
          ...
          <p className="mt-2 text-[10px] text-amber-700 dark:text-amber-400">
            ⚠ Guardala en un lugar seguro. Si la perdés, tenés que crear una
            nueva.
          </p>
        </div>
