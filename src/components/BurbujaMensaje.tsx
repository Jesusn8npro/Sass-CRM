...
export function BurbujaMensaje({ mensaje, idCuenta }: Props) {
  ...
  // ============================================================
  // MEDIA histórica SIN archivo descargado (media_path null)
  // WhatsApp no deja bajar media vieja; mostramos una burbuja limpia
  // (ícono + tipo/nombre) en vez de caer al texto crudo "[imagen histórico]".
  // ============================================================
  if (esMedia) {
    const caption = captionVisible(mensaje);
    const etiqueta =
    ...
    return (
      <div className={`flex ${lado} animate-aparecer`}>
        <div
          className={`min-w-0 max-w-[78%] overflow-hidden rounded-2xl ${radio} border px-4 py-2.5 shadow-sm backdrop-blur-sm ${colores}`}
        >
          <div className="flex items-center gap-2 text-sm [overflow-wrap:anywhere]">
            <svg
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
            ...
            <span className="opacity-90">{caption || etiqueta}</span>
          </div>
          <p
            className={`mt-1 text-[10px] font-medium uppercase tracking-wider ${colorTextoTenue} ${esUsuario ? "" : "text-right"}`}
          >
